...
public class VXMLParser {
	...
	//For storing the files structure with id and vxml files
	private Map<BigDecimal, List<String>> mIDnFiles = new HashMap<BigDecimal, List<String>>();
	//Its has all the combined files in structure
	private VXMLFiles mVxmlFiles = new VXMLFiles();
		...
		if (directory.exists()) {// checks if the folder exists
			...
			if (fListing != null) {
				...
				for (int i = 0; i < fListing.length; i++) {
					...
					parser.traverseFileSystemForVXMLFilesnID(f);
				}
			}
		}

		parser.objectifyVXMLFilesLoaded();

		parser.getVXMLFile().processGrammarFile();
		parser.getVXMLFile().processMessageData();
		System.out.println(parser.getVXMLFile().toString());
	}

	
	public VXMLFiles getVXMLFile(){
		return mVxmlFiles;
	}
	 ...
	 */
	public void traverseFileSystemForVXMLFilesnID(File directory) {
	 ...
	 */	
	public void objectifyVXMLFilesLoaded() {
		//gets all the id
	 ...
	 */	
	private void parseXML2VXMLFile(BigDecimal appID) {
		try {
			...
			if (nList.getLength() > 0) {
				for (int i = 0; i < nList.getLength(); i++) {
					...
					if (nList.item(i).getNodeType() == Node.ELEMENT_NODE) {
						...
						// Audio
						NodeList nlAudio = nElement.getElementsByTagName("audio");
						for (int j = 0; j < nlAudio.getLength(); j++) {							
							for (int j2 = 0; j2 < nlAudio.item(j).getAttributes().getLength(); j2++) {														
								if (nlAudio.item(j).getAttributes().item(j2).getNodeName().trim().equalsIgnoreCase("src")) {											
									//<src><[Message],[""]>
									String filenLoc=nlAudio.item(j).getAttributes().item(j2).getNodeValue();
									String fileName=filenLoc.substring(filenLoc.lastIndexOf("/")+1, filenLoc.length()-1) ;
									String location=filenLoc.substring(0, filenLoc.lastIndexOf("/")) ;
									
									vxmlForm.getMessages().put(fileName, new String[]{location,nlAudio.item(j).getTextContent()});
									break;
								}else if (nlAudio.item(j).getAttributes().item(j2).getNodeName().trim().equalsIgnoreCase("expr")) {
									//<expr><[Message],[""]>
								
									String filenLoc=nlAudio.item(j).getAttributes().item(j2).getNodeValue();
									StringTokenizer st= new StringTokenizer(filenLoc, "+");
									String[] sTokens = new String[st.countTokens()];
									int k =0;
									while(st.hasMoreTokens()){
										sTokens[k++]=st.nextToken();
									}								
									String fileName=sTokens[sTokens.length-1].replaceAll("'", "");
									k=0;
									String location="" ;
									while(k<sTokens.length-1){
										location+=sTokens[k++].trim()+"+";
									}
	 ...
	 */
	private String getElementXML(Node nElement) {
		String sElements = "";
	 ...
	 */
	private SubElement getXMLOfElement(Node nElement, int indent) {
		String sAttributes = "";
		String sElements = "";
